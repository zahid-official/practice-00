...
async function run() {
  try {
    ...
    app.get("/visa", async (req, res) => {
      ...
      console.log(pageNo, pageItems);

      const cursor = visasCollection.find().skip(pageNo * pageItems).limit(pageItems);
    ...
    app.get("/total", async (req, res) => {
      const count = await visasCollection.estimatedDocumentCount();
      res.send({ count });
    });
