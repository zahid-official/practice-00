...
const tokenVerify = (req, res, next) => {
  ...
  // verify token
  jwt.verify(token, process.env.JWT_PrivateKey, (error, decoded) => {
    if(error){
      return res.status(401).send({message: 'Unathorize Access'});
    }
...
async function run() {
  try {
    ...
    app.post('/jwt', async(req, res) => {
      const user = req.body;
      const token = jwt.sign(user, process.env.JWT_PrivateKey, {expiresIn: '1hr'});
  ...
  // read applicatons data with tokenVerify
  app.get('/applications/:email', tokenVerify, async(req, res) => {
    const email = req.params.email;
    console.log(email)
    console.log('decoded',req.decodedToken.email)
    // verify token & api info
